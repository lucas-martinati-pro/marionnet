...
myocamlbuild.ml:
	@($(call READ_CONFIG, ocaml_libraryprefix); \
	echo -e "(* This file is automatically generated. Please don't edit it. *)\n" > $@; \
...
# 	done; \
	for x in $(DIRECTORIES_TO_INCLUDE); do \
		echo -en "A \"-I\"; A \"$$ocaml_libraryprefix/$$x\"; " >> $@; \
	done; \
	echo -e "];;" >> $@; \
...
meta.ml: META
	@(echo "Building $@..." && \
	$(call READ_META, name, version); \
	$(call READ_CONFIG, prefix, ocaml_libraryprefix, libraryprefix, configurationprefix, documentationprefix localeprefix); \
