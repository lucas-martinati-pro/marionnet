...
module Make_menus (Params : sig
  ...
  module Properties = struct
    include Data
    let dynlist () = st#network#get_node_names_that_can_modify ~devkind:`Router ()
...
let make
 ...
 () :'result option =
  ...
  (* --- *)
  in
  (* If the user kept or imported a kernel that cannot boot on this host,
    ...
    let () =
      if Initialization.uml_kernel_broken_on_this_host self#get_kernel then
        ...
        | (best, _) :: _ when not (Initialization.uml_kernel_broken_on_this_host best) ->
            Log.printf3 "Runtime auto-heal: router %S kernel %S unusable on this host, automatically switching to %S\n"
