...
let localeprefix =
  (* Search `marionnet.mo' in, by decreasing priority: the dune-site `locale'
     directories (relocatable, where `dune install' puts the catalogues), then
     MARIONNET_LOCALEPREFIX, then the CONFIGME-baked Meta.localeprefix. The last
     two are the historical fallback, preserved intact. *)
  let localeprefix_candidates =
    let l0 = Sites.locale in
