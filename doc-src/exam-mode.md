...
`scripting/README.md`, § 11. That is how a lab is checked by a script — or by an agent
— rather than by hand.

If what you want is the whole thread — preparing a lab, handing it out, running the session and
marking the copies, with one worked example per command — that is `teacher-guide.md`,
and a complete lab lives in `labs/session-7/`.

And if the marking itself is what you want delegated: `lab-design-skill.md` is the
